...
- Prompt injection via a manipulated Claude Code session tail reaches the agent through both the 20-line watcher scan and the `read_session` tool. Persistence path via `soul.md` is gated by `ROBOOT_SOUL_REVIEW` (§6); the runtime-side injection itself is not blocked.
