...
- **工具调用审批闸**（`tool_guard.py`）—— 与 soul review 同形态，但守的是 *动作* 路径：在 Arcana 派发前拦下 `run_command`（目前唯一被 gate 的攻击面工具）。通过 `ROBOOT_TOOL_APPROVAL` 选模式：`off`（默认，向后兼容）、`log`（危险命令仍允许，但记录到 `.tool_audit/`）、`confirm`（本地控制台 + 移动配对页弹窗 + Telegram inline keyboard 三端弹窗；Telegram 端只有触发命令的那个用户能批准自己的弹窗）。危险检测靠 38 条 curated 正则（rm/dd/sudo/管道接 shell/凭证目录/Roboot at-rest 路径/macOS 持久化等），输入做 NFKC + ANSI 剥离 + 16 KB ReDoS 上限。本机白名单 `~/.roboot/tool_allowlist.json` 放过常用命令；含元字符的条目（`;`、`&`、反引号、`$(`、`||` 等）查表时直接忽略，避免用户写 `prefix: "ls; rm -rf"` 自己卸自己的防。挂在 Arcana 的 `runtime._tool_gateway.confirmation_callback` 上 —— 被 gate 的工具只需在 `@arcana.tool(...)` 加 `requires_confirmation=True`。同时移除了 `tools/shell.py` 里旧的 8 条子串黑名单（之前对未匹配命令返回硬编码字符串误导 agent，且没有移动端 / 审计路径）。
...
- **`ROBOOT_TOOL_APPROVAL` / `ROBOOT_SOUL_REVIEW` 部署矩阵** —— `SECURITY.md` §7 + `config.example.yaml` 内联给出按实际暴露面选模式的决策表：localhost-only daemon 用 `log`；Telegram、relay 配对、读不可信 `session_watcher` 任一开启就用 `confirm`。`off` 是向后兼容默认值，不是推荐。承接 D5 smoke 得出的 "log 是新装机底线" 立场。

### 修复
- **`ROBOOT_TOOL_APPROVAL=confirm` 下 WS 接收循环死锁**（`server.py`）—— 本地控制台原来直接 `await handle_chat`，被 gate 的回合还在等 modal 决策时，用户点的 `tool_approval_decision` 帧根本读不到，要等到 30 秒 gate 自己 timeout、handle_chat 返回，循环才回来读，那时 future 已 resolved，每次点击都是 stale → audit 全是 `TIMEOUT`。改成 reader/runner 两个 task 通过 `asyncio.Queue` 解耦：决策/控制帧立刻 resolve，用户输入帧排队、runner 串行消费，single-session-per-WS 契约保留。Relay（`async for ws → create_task` 每帧）和 Telegram（PTB per-update dispatcher）走的就是独立 task 模型，从来没这个 bug —— 只有本地 WS 路径串行卡在一个协程里。
- **被回放的幻觉助手回复让 gate 形同虚设**（`memory.py` + `tools/soul.py` + `soul.md`）—— DeepSeek 习惯于对破坏性命令偷懒，直接回 "已经清理掉了" 但根本没调 `shell`。这条假的 assistant turn 落进 `chat_store`，下次 `replay_history` 又重新喂回去，模型把自己的过往话当成事实、跳过真的工具调用，于是 gate 永远不会触发。两层修：(a) `memory._seed_messages` 和 `_fallback_context_summary` 在 replay 时整体丢掉 assistant 消息 —— 用户消息是请求的权威记录，助手回复可能是谎话；(b) `tools/soul.py:build_personality` 拼出来的 system prompt 明确写出"用户每次发动作请求都必须真调工具，不许凭对话历史里你自己说过'已经做了'就跳过"。
