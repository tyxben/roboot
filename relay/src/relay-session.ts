...
export class RelaySession implements DurableObject {
  ...
  async webSocketMessage(ws: WebSocket, message: string | ArrayBuffer): Promise<void> {
    ...
    // These are plaintext by design so the relay can route them without keys.
    ...
    // Relay only forwards whitelisted message types — application traffic
    // must travel as `encrypted` envelopes. Silently dropping keeps the DO
    // simple and prevents plaintext leakage bugs upstream.
