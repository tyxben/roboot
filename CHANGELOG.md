...
- **Tool approval gate** (`tool_guard.py`) — same shape as soul review, but for the *action* path: gates `run_command` (today's only attack-prone tool) before Arcana dispatches it. Mode selected by `ROBOOT_TOOL_APPROVAL`: `off` (default, backwards compatible), `log` (dangerous calls allowed but recorded to `.tool_audit/`), `confirm` (modal on local console + mobile pair-page + Telegram inline keyboard; only the triggering Telegram user can answer their own modal). Danger detection via 38 curated regexes (rm/dd/sudo/pipe-to-shell/credential dirs/Roboot at-rest paths/macOS persistence/etc.) with NFKC + ANSI strip + 16 KB ReDoS guard. Per-machine allowlist at `~/.roboot/tool_allowlist.json` for routine commands; metachar-containing entries silently rejected so a user can't disarm the gate by accident. Wired into Arcana via `runtime._tool_gateway.confirmation_callback` — gated tools just declare `requires_confirmation=True`. Replaces the legacy 8-entry substring blacklist in `tools/shell.py` (which gave the agent a misleading hardcoded reply for unmatched commands and offered no mobile/audit trail).
...
- **Deployment matrix for `ROBOOT_TOOL_APPROVAL` / `ROBOOT_SOUL_REVIEW`** in `SECURITY.md` §7 + inline in `config.example.yaml`: localhost-only daemon → `log`; any of Telegram/relay/untrusted-`session_watcher` → `confirm`. `off` is backwards-compat default, not a recommendation. Mirrors the `log`-as-floor stance from D5 smoke results.

### Fixed
- **WS receive loop deadlock under `ROBOOT_TOOL_APPROVAL=confirm`** (`server.py`) — the local-console handler `await`ed `handle_chat` directly, so any `tool_approval_decision` frame the user sent during a gated turn sat unread until the gate's 30 s timeout fired and the future had already resolved. Every modal click came back stale → audit always showed `TIMEOUT`. Split the WS handler into a reader loop and a chat-runner task fed by `asyncio.Queue`: decision/control frames resolve immediately while user-input frames queue and the runner serializes them one-at-a-time, preserving the single-session-per-WS contract. Relay (`async for ws → create_task` per frame) and Telegram (PTB per-update dispatcher) were never affected — only the local WS path serialized everything in one coroutine.
- **Replay of hallucinated assistant turns broke the gate's effectiveness** (`memory.py` + `tools/soul.py` + `soul.md`) — DeepSeek likes to dodge destructive commands by emitting a fake "已经清理掉了" without ever calling `shell`. That assistant turn lands in `chat_store`, gets re-seeded on the next `replay_history`, and the model treats its own past words as ground truth — skipping the real tool call so the gate never fires. Two-layer fix: (a) `memory._seed_messages` and `_fallback_context_summary` now drop assistant turns entirely on replay, since user messages are the authoritative records of intent and assistant replies might be lies; (b) the system prompt built by `tools/soul.py:build_personality` spells out that every explicit user request requires a fresh tool call regardless of what the agent claimed earlier in the conversation.
